        ...
        closeHeaderTop: function () {
            ...
            // if (closeHeaderTopElm.length && closeHeaderTopElm.is(':visible')) {
                if ($.cookie('headerTop') == 'closed') {
                    headerTopEml.remove();
                  	$('.wrapper-header').addClass('top-header-anou');
                } else {
...
//                     productItemElm.find('[data-value="' + title + '"]').find('label').trigger('click');
                ...
                if (self.attr('disabled') !== "disabled") {
                    ...
                    switch (window.ajax_cart) {
                        ...
                        case "inline":
                            ...
                            ella.doAjaxAddToCartInline(data, title, image);
        ...
        closeSuccessModal: function () {
      	...
      	doAjaxAddToCartInline: function(data, title, image) {
            $.ajax({
                type: "POST",
                url: "/cart/add.js",
                data: data,
                ...
                success: function (rData) {
                  jQuery.getJSON('/cart.js', function(cart){
                    var ajaxCartModal = $('#atc-banner');
                  	modalContent = ajaxCartModal.find('.atc-box-product'),
                  	modalContent1 = ajaxCartModal.find('.atc-box-cart');
                      console.log(cart);
                    modalContent1.find('.atc-total-cart-qty').html(cart.item_count);
                    $('.cartCount').html(cart.item_count);
                    modalContent1.find('.atc-subtotal-price').html(Shopify.formatMoney(cart.items_subtotal_price, window.money_format))
                  });
                  ...
                  var ajaxCartModal = $('#atc-banner');
                  	modalContent = ajaxCartModal.find('.atc-box-product'),
                  	modalContent1 = ajaxCartModal.find('.atc-box-cart');
                    modalContent.find('.atc-pd-title').html(ella.translateText(title));
                    modalContent.find('.ajax-product-image').attr('src', image);
                    modalContent.find('.atc-pd-price-quantity').html(data.quantity);
                    modalContent.find('.atc-pd-price-value').html(Shopify.formatMoney(rData.price, window.money_format));
